...
namespace Jimmy{
    class Net{ // Class for training and useage of Network
        ...
        const double& getResult(int) const; // returns output vale of the indexed neuron in last layer
        const std::vector<std::reference_wrapper<double>>& getResult() const; // return vector on the

        Net(std::vector<unsigned int>, const Jimmy::TransferFunction&, const Jimmy::LossFunction&, double); // Create network with, n naurons in each layer
    };
}
