...
void Jimmy::Critic::backProp(double power, int treat){
    ...
    // output gradient
    for(int i = 0; i < this->watchListNeurons.size(); i++){
        this->watchListNeurons[i].get().gradient = this->networkPointer->learningRate * power * (desiredValues[i] - this->watchListNeurons[i].get().outValue)  * this->networkPointer->transFunc.runDerivative(this->watchListNeurons[i].get().outValue);
    }
    ...
    for(int j = 0; j < this->networkPointer->layers[i].neurons.size(); j++){
        ...
        this->networkPointer->layers[i][j].gradient = sumGradientWeights * this->networkPointer->transFunc.runDerivative(this->networkPointer->layers[i][j].outValue);
    }
